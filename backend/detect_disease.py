...
MODEL_ID = "disease-detection-biciz/2"
...
def run_inference(image_path):
    ...
    print(f"Running disease inference on: {image_path}")
...
def save_analyzed_image(image_path, result, output_path=None):
    ...
    if output_path is None:
        ...
        output_path = base_path.parent / f"{base_path.stem}_disease_analyzed.jpg"
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python detect_disease.py <image_path> [output_path]")
        print("Example: python detect_disease.py image.jpg")
        sys.exit(1)
