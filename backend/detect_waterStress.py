...
def remap_water_stress_labels(result):
    """Map model classes into simplified water stress labels."""
    if result is None:
        return None

    for prediction in result.get("predictions", []):
        original_class = str(prediction.get("class", "Unknown"))
        normalized = original_class.strip().lower()
        mapped = LABEL_MAP.get(normalized, "dry")

        # Preserve original class for debugging while exposing merged label.
        prediction["original_class"] = original_class
        prediction["class"] = mapped

    predictions = result.get("predictions", [])
    if predictions:
        # Keep only the highest-confidence detection to avoid duplicate dry boxes.
        top_prediction = max(predictions, key=lambda item: item.get("confidence", 0))
        result["predictions"] = [top_prediction]
...
def run_inference(image_path):
    """
    Run water stress inference on an image using Roboflow API.
    ...
    print(f"Running water stress inference on: {image_path}")

    try:
        result = CLIENT.infer(image_path, model_id=MODEL_ID)
        print("Inference completed successfully via serverless endpoint")
        return remap_water_stress_labels(result)
...
def save_analyzed_image(image_path, result, output_path=None):
    ...
    if output_path is None:
        ...
        output_path = base_path.parent / f"{base_path.stem}_water_stress_analyzed.jpg"
    ...
    for prediction in result.get("predictions", []):
        ...
        cv2.rectangle(image, (x1, y1), (x2, y2), (255, 102, 0), 4)
...
# Merge Roboflow classes into water-stress labels.
LABEL_MAP = {
    "healthy": "healthy",
    "bacterial_spot": "dry",
    "early_blight": "dry",
    ...
    "leaf_mold": "dry",
    "mosaic_virus": "dry",
    "septoria_leaf_spot": "dry",
    "target_spot": "dry",
    "two-spotted_spider_mite": "dry",
    "yellow_leaf_curl_virus": "dry",
}


def main():
    if len(sys.argv) < 2:
        print("Usage: python detect_waterStress.py <image_path> [output_path]")
        print("Example: python detect_waterStress.py your_image.jpg")
        sys.exit(1)
